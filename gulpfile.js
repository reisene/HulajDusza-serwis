...
gulp.task('html', function () {
  ...
  return gulp
    .src([path.join(paths.src, '**/*.html')]) // Przetwarza wszystkie pliki HTML w src i podfolderach
...
gulp.task('css', function () {
  try {
    ...
    return gulp
      ...
      .pipe(postcss([autoprefixer(), postcssPresetEnv({ stage: 3 })]))
...
gulp.task('watchPosts', async function () {
  ...
  await fs.copyFile(
    'src/js/modules/post-paths.js',
    'public_html/js/modules/post-paths.js'
  )
})

gulp.task('js', () => {
  return gulp.src(paths.js).pipe(gulp.dest(path.join(paths.dest, 'js')))
})
